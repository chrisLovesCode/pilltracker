...
class MediRoutineE2ETest {
    ...
    private fun dumpLogcatFiltered(): String {
        return try {
            val filtered = device.executeShellCommand(
                "sh -c \"logcat -d | grep -E 'Capacitor/Console|\\\\[Notifications\\\\]|\\\\[Debug\\\\]|\\\\[Print\\\\]|LocalNotifications|Database Initialization Failed|no such table|\\\\[DB\\\\]|\\\\[Main\\\\]|AndroidRuntime|CapacitorSQLite|MediRoutinePrint|printCurrent|PrintManager|sqlite|SQL|FATAL|ERROR' | tail -n 300\""
            ).trim()
    ...
    private fun getMedicationProgressValue(medicationId: String, timeoutMs: Long = 30000): Double {
        val selector = "[data-testid='medication-next-dose-progress-fill-${medicationId}']"
        waitForCss(selector, timeoutMs)
        val raw = captureAttrByJs(selector, "data-progress", timeoutMs).trim()
        val parsed = raw.toDoubleOrNull()
        if (parsed == null) {
            throw AssertionError("Invalid medication progress value for ${medicationId}: \"$raw\"")
        }
        return parsed
    }
    ...
    private fun setNowOverrideForTests(value: String?) {
        ...
        val script = """
            if (typeof window.__mediroutineSetNowForTests !== 'function') return "NO_HOOK";
            return window.__mediroutineSetNowForTests($jsValue);
        """.trimIndent()
    ...
    fun test07_dueBadgeReappearsForLaterSlotAfterEarlierTrack() {
        ...
        val midBetweenSlotsCal = (secondSlotCal.clone() as Calendar).apply {
            add(Calendar.MINUTE, -45)
            set(Calendar.SECOND, 0)
            set(Calendar.MILLISECOND, 0)
        }
        val secondDueCheck = localIsoString(secondDueCheckCal)
        val midBetweenSlots = localIsoString(midBetweenSlotsCal)
        println("[test07] firstSlot=$firstSlot secondSlot=$secondSlot firstDueCheck=$firstDueCheck midBetweenSlots=$midBetweenSlots secondDueCheck=$secondDueCheck")
        ...
        val firstDueProgress = getMedicationProgressValue(medId, 45000)
        if (firstDueProgress > 0.05) {
            fail("Expected progress to be near 0 while first slot is due, got $firstDueProgress")
        }
        ...
        // Between the two slots, medication is not yet due and progress should be in-between.
        setNowOverrideForTests(midBetweenSlots)
        waitForCssAbsent("[data-testid='medication-due-${medId}']", 45000)
        val midProgress = getMedicationProgressValue(medId, 45000)
        if (midProgress <= 0.15 || midProgress >= 0.85) {
            fail("Expected progress between slots to be dynamic (0.15..0.85), got $midProgress")
        }
        ...
        waitForCss("[data-testid='medication-due-${medId}']", 45000)
        waitForCss("[data-testid='medication-card-${medId}'] [aria-label='last-intake']", 45000)
        val secondDueProgress = getMedicationProgressValue(medId, 45000)
        if (secondDueProgress > 0.05) {
            fail("Expected progress to be near 0 while second slot is due, got $secondDueProgress")
        }
